...
answer in its own file identical to the record), then f4_checks.mjs (6, with the real extension: the chat's models,
...
def test_open_webui_is_webllms_face_on_the_real_screen(tmp_path):
    ...
    try:
        ...
        out = subprocess.run(["node", str(ROOT / "tests" / "openwebui" / "f5_checks.mjs")], capture_output=True, text=True,
                             timeout=300, env={**os.environ, "OW_URL": ow_url, "OW_EMAIL": email, "OW_PASSWORD": password,
